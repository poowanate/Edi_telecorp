...
function table() {
  ...
  const [searchtxtinvoieno, setsearchtxtinvoieno] = useState('');
  ...
  const searchpO_NO =()=>{
    ...
    if(searchtxtpono!=''){
      setshowtable(datatable2.filter((x => x.pO_NO == searchtxtpono)))
  ...
  const searchinvoicE_NO =()=>{
    ...
    if(searchtxtinvoieno!=''){
      setshowtable(datatable2.filter((x => x.invoicE_NO == searchtxtinvoieno)))
  ...
  <div className="">
  <div className="bg-gray-100 flex items-center rounded-full shadow-xl">
    <input onChange={(e)=>setsearchtxtinvoieno(e.target.value)} className="bg-gray-100 rounded-l-full w-full py-4 px-6 text-gray-700 leading-tight focus:outline-none" id="search" type="text" placeholder="Search" />
